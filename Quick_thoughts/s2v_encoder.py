...
from Quick_thoughts import s2v_model
...
class s2v_encoder(object):
    ...
    def __init__(self, config, uniform_init_scale,
    ...
    """
        ...
        model = s2v_model.s2v(model_config, self.uniform_init_scale,
                              self.input_file_pattern, self.shuffle_input_data, self.input_queue_capacity,
                              self.num_input_reader_threads, self.batch_size, self.dropout,
                              self.dropout_rate, self.context_size, mode="encode")
        model.build_enc()
        self._embeddings = model.word_embeddings  # encode返回包含两个字典的列表，key:词，value：index
    ...
    """
        ...
        vocab_configs = self.config.vocab_configs  # 是包含一个字典的列表

        tf.logging.info("the vocab_configs length is %d, and the self._embeddings length is %d",
                        len(vocab_configs), len(self._embeddings))
        ...
        for i in range(len(vocab_configs)):
            ...
            if v.mode == "expand":
                ...
                data_proc = self._preprocess(data, use_eos, self._embeddings[2 * i])  # 利用字典获取索引
